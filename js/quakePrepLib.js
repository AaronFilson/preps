...
var UserInfo = function () {
  ...
  this.medList = [false, "I need medication or a medical device. Everyone should have an emergency kit just in case, and the emergency kit should keep extra medication that may last for weeks if not months after an earthquake. The contents of an emergency kit should contain (but not limited to): \
    <ul> \
...
UserInfo.prototype.getFromStore = function(username) {
  ...
  if(temp != null) {
      ...
      displayTheLists(); // need method call
    }
};
...
var TableMaker = function(username, age, gender, address, contactNo, meetUpLoc, extraInfo, contactColumn) {
  ...
  this.contactColumn = contactColumn; //reference to page table
...
//The new entry submit object
var newFormSubmit = function(e) {
  e.preventDefault();
  ...
  var profile = newUser.value.toUpperCase();
  
  setToStore(); //need a method call
  //reset fields
  newUser.value = null;
  newAge.value = null;
